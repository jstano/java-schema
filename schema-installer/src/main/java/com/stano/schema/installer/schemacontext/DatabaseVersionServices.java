...
public class DatabaseVersionServices {
    ...
    TransactionalExecutor.withConnection(connection)
        .execute(
            () -> {
              ...
              try (Statement statement = connection.createStatement()) {
                statement.execute(
                    String.format("update databaseversion set version = '%s'", version.toString()));
              }
            });
  }
  ...
  private void ensureDatabaseVersionTableExists(Connection connection) throws SQLException {
    ...
    try (Statement statement = connection.createStatement()) {
      statement.executeUpdate(
          getDatabaseVersionCreateTableSql(DriverType.fromConnection(connection)));
  ...
  private String getDatabaseVersionCreateTableSql(DriverType driverType) {
    if (driverType == DriverType.MSSQL) {
      return "create table dbo.databaseversion (version varchar(10) not null,constraint"
          + " pk_databaseversion primary key (version))";
    } else {
      return "create table databaseversion (version varchar(10) not null,constraint"
          + " pk_databaseversion primary key (version))";
    }
  }
}
